...
import com.ydys.qmb.ui.custom.LineChartMarkView;
 ...
 */
public class LineChartLevelManager {
    ...
    public LineChartLevelManager(LineChart lineChart) {
...
//        description.setText("需要展示的内容");
     ...
     * @param color    曲线颜色
     */
    public void showLineChart(final List<TrendInfo> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            TrendInfo data = dataList.get(i);
            /**
             * 在此可查看 Entry构造方法，可发现 可传入数值 Entry(float x, float y)
             * 也可传入Drawable， Entry(float x, float y, Drawable icon) 可在XY轴交点 设置Drawable图像展示
             */
            Entry entry = new Entry(i, (float) data.getTrendValue());
            entries.add(entry);
        }
        ...
        xAxis.setValueFormatter(new ValueFormatter() {
            @Override
            public String getFormattedValue(float value) {
                String tradeDate = dataList.get((int) value % dataList.size()).getYears();
                return tradeDate;
            }
        });
     ...
     * 添加曲线
     */
    public void addLine(List<TrendInfo> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            TrendInfo data = dataList.get(i);
            Entry entry = new Entry(i, (float) data.getTrendValue());
     ...
     */
    public void resetLine(int position, List<TrendInfo> dataList, String name, int color) {
