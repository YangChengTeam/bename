...
public class VipInfoModelImp extends BaseModel implements VipInfoModel<PriceInfoRet> {
    ...
    @Override
    public void vipPrice(IBaseRequestCallBack<PriceInfoRet> iBaseRequestCallBack) {

        RequestBody requestBody = RequestBody.create(MediaType.parse("application/json"), "");

        mCompositeSubscription.add(vipInfoService.vipPrice(requestBody)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(new Subscriber<PriceInfoRet>() {
                    @Override
                    public void onStart() {
                        super.onStart();
                    ...
                    @Override
                    public void onNext(PriceInfoRet priceInfoRet) {
                        //回调接口：请求成功，获取实体类对象
                        iBaseRequestCallBack.requestSuccess(priceInfoRet);
                    }
                }));
    }
